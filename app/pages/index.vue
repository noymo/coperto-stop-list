...
const selectedItem = computed<MenuItem | null>(() => {
  ...
  return (
    menuItems.value?.find((item) => item.id === uiStore.selectedItemId) ?? null
  );
});
...
const savingIds = computed<string[]>(() => {
  ...
  if (resumeMutation.isPending.value && resumeMutation.variables.value) {
    ids.push(resumeMutation.variables.value);
  }
...
<template>
  <main class="min-h-screen bg-[#F6F3EE] p-10 text-[#171512]">
    <div class="mx-auto max-w-6xl">
      ...
      <div v-if="isPending" class="mt-8 rounded-xl bg-white p-6">
        Загрузка меню...
      </div>

      <div
        v-else-if="isError"
        class="mt-8 rounded-xl border border-red-200 bg-red-50 p-6 text-red-700"
      >
      ...
      <div
        ...
        class="mt-8 rounded-xl bg-white p-6 text-neutral-500"
      >
